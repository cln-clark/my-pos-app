        ...
        try {
            ...
            if (user) {
                setCurrentUser({
                    ...user,
                    email: user.email || '',
                    role: user.role || { id: 0, name: 'Unknown' }
                });
                return true;
            }
            return false;
   ...
   const addToCart = useCallback((product: Product, quantity: number) => {
        ...
        if(quantity <=0 ){
            ...
            setCart((prev) => prev.map((item) =>
                item.product.id === productId ?
                    { ...item, quantity, discountQty: item.discountCode ? quantity : item.discountQty }
                    : item
            ))
        }
    }, [])
    ...
    const createTransaction = useCallback(
        ...
        try {
            ...
            const items = transaction.items.map((item: TransactionItem, index: number) => {
                const now = new Date();
                ...
                return {
                    ...
                    charge_amt: 0,
                    total_portion_qty: item.quantity,
            ...
            const transactionData = {
                ...
                txn_mode_code: txn_mode_code,
                business_date: transaction.timestamp.toLocaleDateString(),
                cash_amount_paid: cashAmountPaid || undefined,
            ...
            const beneficiaryInfoItem = transaction.items.find((item: TransactionItem) => item.beneficiaryId && item.beneficiaryName);
            ...
            const discountCode = transaction.items.find((item: TransactionItem) => item.discountCode)?.discountCode || null;
            ...
            const zxReadingData = {
                ...
                store_code: storeCode,
                terminal_id: terminalId,
                transaction_no: result.id as unknown as number,
                ...
                sr_pwd_total_amount: transaction.items
                    .filter((item: TransactionItem) => item.isVatExempt)
                    .reduce((sum: number, item: TransactionItem) => sum + (item.quantity * item.product.price), 0), // Gross amount of SC/PWD items
                sr_pwd_vat_exempt_sale: transaction.items
                    .filter((item: TransactionItem) => item.isVatExempt)
                    .reduce((sum: number, item: TransactionItem) => sum + (item.quantity * item.product.price), 0) / 1.12, // VAT exempt amount
                sr_pwd_count: transaction.items.filter((item: TransactionItem) => item.isVatExempt).length,
                cashier_user_code: transaction.cashierUserCode.toString(),
            ...
            await createPosZxReading(zxReadingData).catch(err => {
                console.error('Error creating ZX reading:', err);
                toast.error('Failed to create ZX reading record');
            });
            ...
            toast.error('Failed to create transaction. Using offline mode.');
