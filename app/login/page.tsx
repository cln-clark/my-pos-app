    ...
    import { DayStartOverlay } from "@/components/pos/day-start-overlay";
    ...
    export default function LoginPage() {
      ...
      const [showDayStartOverlay, setShowDayStartOverlay] = useState(false);
      ...
      useEffect(() => {
        // Check if day start was just performed
        const dayStart = searchParams.get('dayStart');
        if (dayStart === 'true') {
          setShowDayStartOverlay(true);
          // Clean up the URL
    ...
    return (
      <div className="flex min-h-screen w-full items-center justify-center p-4 md:p-10 bg-muted/40 relative">
        ...
        <div className={`absolute top-4 left-4 text-sm text-muted-foreground ${fromMain ? 'ml-14' : ''}`} suppressHydrationWarning>
        ...
        <DayStartOverlay
          open={showDayStartOverlay}
          onClose={() => setShowDayStartOverlay(false)}
        />
      </div>
    );
  }
